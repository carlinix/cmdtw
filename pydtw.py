...
from cmdtw.cymdtw import cymdtw, Settings


class dtw(cymdtw):
    def __init__(self, ref, query, settings=Settings()):
        cymdtw.__init__(self, ref, query, settings)
        self._ref = ref
        self._query = query
...
if __name__ == '__main__':
    d = dtw(np.random.rand(10)[:, np.newaxis], np.random.rand(10)[:, np.newaxis], Settings(compute_path=True))
